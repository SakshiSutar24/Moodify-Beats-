...
document.querySelectorAll(".song").forEach(song => {
    song.addEventListener("click", function() {
        ...
        window.location.href = `pagglait_index.php?title=${encodeURIComponent(title)}&artist=${encodeURIComponent(artist)}&src=${encodeURIComponent(src)}&image=${encodeURIComponent(image)}`;
    });
});
document.addEventListener('DOMContentLoaded', function() {
    ...
    songs.forEach(song => {
      song.addEventListener('click', function() {
        window.location.href = 'pagglait_index.php';
      });
    });
  });